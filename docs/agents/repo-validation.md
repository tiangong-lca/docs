...
lastReviewedNote: "Reviewed for Docs #210 SEO Plan v2 (root-corrected): `/` is the canonical Chinese home and `/zh/` is only a permanent provider redirect (one alias; retired paths keep their 404s), so `/zh/` is no longer a canonical, hreflang or sitemap target. The sitemap lists each canonical URL once with reciprocal alternates and a default-language `x-default`, and omits `lastmod` instead of stamping one build epoch on unrelated pages. Page descriptions use the authored frontmatter first, else the page's own structured prose (code, JSX, tables and navigation rejected, Unicode-safe truncation); a page with neither publishes no page-specific description, inherits the layout site description, and is reported as editorial content debt by `verify:out` rather than counted as coverage. Four-locale sources, routes, search, presentation and static export are unchanged; EdgeOne-layer redirect proof and production samples remain pending."
related:
  - AGENTS.md
  - .docpact/config.yaml
