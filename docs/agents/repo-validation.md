...
- Source-derived publication inventory: search records and llms must equal the full indexable MDX URL set; the sitemap must equal every real source page plus the default landing page and each non-default locale landing page. The `/zh/` home alias is a permanent redirect, so it appears in no sitemap entry and in no hreflang map, while remaining an exported page and keeping its retired-path 404 neighbours. Sitemap entries carry no `lastmod` while the build exposes a single source epoch. Static search must parse the advanced multilingual `docs.docs` store, require exact normalized URLs from `type: page` records, and validate their locale. Substring matches, child URLs, heading/text links, and token/ID metadata cannot prove a missing guide root. Keep the parent-missing/child-present regression and malformed-store/locale tests. Static page records must equal the full source inventory, and retired URLs must be absent from page, heading, and text records. Verify guide discovery in production locale-filtered Algolia. Ordinary additions leave the retained route and retired-path baselines unchanged, and no redirect beyond the home alias may be added.
...
- Static SEO artifact check: the required-check job checks the generated `scripts/vendor/workspace-seo/` snapshot against its `manifest.json` and runs that same local checker against the built `out/` (`--origin http://localhost:3000`, `--root out`, `--artifact-indexing disabled`) instead of rebuilding, and uploads `seo-report.json` with `if: always()`. CI must fetch no external action and hold no token, because the workspace source repository is private; the snapshot is read-only here and is refreshed only by the workspace-side export. A matching local digest proves the committed bytes are intact, not their origin — only the workspace export, comparing the selected Git blob, attests the private source commit. Provider ownership verification is gated here too: with `BAIDU_SITE_VERIFICATION` set the sampled pages must carry that exact code, and with it unset no page may publish one. CI sets a non-production fixture value on the build step, so the configured path is exercised end to end while the real code stays a deployment-only value that is never committed. The marker is published at build time, because a static export freezes metadata once.
- Metadata or route changes: inspect generated HTML for canonical, `x-default`, all real locale alternatives, and Open Graph image metadata; confirm sitemap entries and negative 404 contracts. `scripts/seo-policy.test.mjs` pins the home/alias mapping, the authored/derived/unresolved summary rules, the single-alias redirect contract, and the route manifest flags without a browser; the complete build's `verify:out` proves the exported pages and sitemap. `verify:out` also measures description coverage and prints the sorted list of URLs that have no page-specific description: those pages inherit the layout site description, so they are **editorial content debt to report, never a gate to hide and never a reason to noindex or block a page**. Do not claim description coverage from a generic site-level fallback, and do not weaken an existing check to silence the list. Prove the `/zh/` redirect at the hosting layer (EdgeOne redirect rule or a real deployed request) because the local static preview cannot execute it, and confirm no second redirect exists.
