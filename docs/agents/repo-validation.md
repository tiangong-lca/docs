...
- Metadata or route changes: inspect generated HTML for canonical, `x-default`, all real locale alternatives, and Open Graph image metadata; confirm sitemap entries and negative 404 contracts. `scripts/seo-policy.test.mjs` pins the home/alias mapping, the derived-description rules, the single-alias redirect contract, and the route manifest flags without a browser; the complete build's `verify:out` proves the exported pages and sitemap. Prove the `/zh/` redirect at the hosting layer (EdgeOne redirect rule or a real deployed request) because the local static preview cannot execute it, and confirm no second redirect exists.
